...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(RAODVTYPE_RREQ);
...
RoutingProtocol::RecvRaodv(Ptr<Socket> socket)
{
    ...
    case RAODVTYPE_REVRREQ: {
        RecvReverseRequest(packet, receiver, sender);
        break;
    }
        // code
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        // code
        SendReverseRequest(rreqHeader, toOrigin);
        // code
        return;
    }
    /*
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(RAODVTYPE_RREP);
    packet->AddHeader(tHeader);
...
// code
void
RoutingProtocol::SendReverseRequest(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    NS_LOG_FUNCTION(this << toOrigin.GetDestination());
    /*
     * Destination node MUST increment its own sequence number by one if the sequence number in the
    ...
    RevRreqHeader revRreqHeader(/*flags=*/0,
                                /*reserved=*/0,
                                /*hopCount=*/0,
                                /*requestID=*/rreqHeader.GetId(),
                                /*dst=*/rreqHeader.GetDst(),
                                /*dstSeqNo=*/rreqHeader.GetDstSeqno(),
                                /*origin=*/toOrigin.GetDestination(),
                                /*originSeqNo=*/m_seqNo);
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        Ptr<Packet> packet = Create<Packet>();
        SocketIpTtlTag tag;
        tag.SetTtl(rreqHeader.GetHopCount());
        packet->AddPacketTag(tag);
        packet->AddHeader(revRreqHeader);
        TypeHeader tHeader(RAODVTYPE_REVRREQ);
        packet->AddHeader(tHeader);
        // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())
        ...
        NS_LOG_DEBUG("Send RREQ with id " << revRreqHeader.GetId() << " to socket");
        m_lastBcastTime = Simulator::Now();
        Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
                            &RoutingProtocol::SendTo,
                            this,
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(RAODVTYPE_RREP);
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(RAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
...
// code
void
RoutingProtocol::RecvReverseRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    NS_LOG_FUNCTION(this << " src " << sender);
    RevRreqHeader revRreqHeader;
    p->RemoveHeader(revRreqHeader);
    Ipv4Address dst = revRreqHeader.GetDst();
    NS_LOG_LOGIC("RREP destination " << dst << " RREP origin " << revRreqHeader.GetOrigin());

    uint8_t hop = revRreqHeader.GetHopCount() + 1;
    revRreqHeader.SetHopCount(hop);

    // If RREP is Hello message
    if (dst == revRreqHeader.GetOrigin())
    {
        // ProcessHello(revRreqHeader, receiver);
        return;
    }
    ...
    // Acknowledge receipt of the RREP by sending a RREP-ACK message back
    // if (revRreqHeader.GetAckRequired())
    // {
    //     SendReplyAck(sender);
    //     revRreqHeader.SetAckRequired(false);
    // }
    NS_LOG_LOGIC("receiver " << receiver << " origin " << revRreqHeader.GetOrigin());
    ...
    RoutingTableEntry toOrigin;
    if (!m_routingTable.LookupRoute(revRreqHeader.GetOrigin(), toOrigin) ||
        toOrigin.GetFlag() == IN_SEARCH)
    {
        return; // Impossible! drop.
    }
    ...
    // Update information about precursors
    if (m_routingTable.LookupValidRoute(revRreqHeader.GetDst(), toDst))
    {
        toDst.InsertPrecursor(toOrigin.GetNextHop());
        m_routingTable.Update(toDst);
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(RAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(RAODVTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
