...
// code
int cmd_node_speed = 10;
int cmd_node_number = 20;
int cmd_packets_per_second = 100;
int cmd_packet_size = 64;
double network_throughput = 0;
int total_packets_sent = 0;
int total_packets_recieved = 0;
int total_packets_droped = 0;
double total_bits_recieved = 0;
double total_time = 0;
double total_delay = 0;
std::string report_CSVfileName{"report_aodv.csv"};
// code
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    // code
    cmd.AddValue("NumberOfNodes", "The number of nodes for the experiment", cmd_node_number);
    cmd.AddValue("SpeedOfNodes", "The speed fo nodes for the experiment", cmd_node_speed);
    cmd.AddValue("NumberOfPacketsPerSecond",
                 "The number of packets per second for the experimnet",
                 cmd_packets_per_second);
    cmd.AddValue("PacketSize", "The packet size for the experiment", cmd_packet_size);
    // code
    cmd.Parse(argc, argv);
...
main(int argc, char* argv[])
{
    ...
    experiment.Run();
    // code
    total_packets_droped = total_packets_sent - total_packets_recieved;
    std::ofstream report_out(report_CSVfileName, std::ios::app);
    report_out << "AODV" << "," << cmd_node_number << "," << cmd_packets_per_second << ","
               << cmd_node_speed << "," << total_bits_recieved * 1.0 / total_time << ","
               << total_packets_recieved * 1.0 / total_packets_sent << ","
               << total_packets_droped * 1.0 / total_packets_sent << "," << total_delay
               << std::endl;
    report_out.close();
    // code
    return 0;
...
RoutingExperiment::Run()
{
    ...
    // int nWifis = 50;
    int nWifis = cmd_node_number;

    double TotalTime = 200.0;
    total_time = TotalTime;
    // std::string rate("2048bps");
    std::string rate(std::to_string(cmd_packets_per_second * 8 * cmd_packet_size) + "bps");
    std::string phyMode("DsssRate11Mbps");
    std::string tr_name("manet-routing-compare");

    // int nodeSpeed = 20; // in m/s
    int nodeSpeed = cmd_node_speed; // in m/s
    int nodePause = 0;              // in s

    // Config::SetDefault("ns3::OnOffApplication::PacketSize", StringValue("64"));
    Config::SetDefault("ns3::OnOffApplication::PacketSize",
                       StringValue(std::to_string(cmd_packet_size)));
    ...
    if (m_flowMonitor)
    {
        ...
        std::map<FlowId, FlowMonitor::FlowStats> stats_mp = flowmon->GetFlowStats();
        for (auto& stat_flow : stats_mp)
        {
            FlowMonitor::FlowStats stats = stat_flow.second;
            total_bits_recieved += stats.rxBytes * 8.0;
            total_packets_recieved += stats.rxPackets;
            total_packets_sent += stats.txPackets;
            total_packets_droped += stats.lostPackets;
            total_delay += stats.delaySum.GetSeconds();
        }
        // code
...
// double total_time = 0;
// double total_delay = 0;
// std::string report_CSVfileName{"report_raodv.csv"};
// // code
...
//     total_packets_droped = total_packets_sent - total_packets_recieved;
//     std::ofstream report_out(report_CSVfileName, std::ios::app);
//     report_out << "R-AODV" << "," << cmd_node_number << "," << cmd_packets_per_second << ","
//                << cmd_node_speed << "," << total_bits_recieved * 1.0 / total_time << ","
//                << total_packets_recieved * 1.0 / total_packets_sent << ","
//                << total_packets_droped * 1.0 / total_packets_sent << "," << total_delay
//                << std::endl;
